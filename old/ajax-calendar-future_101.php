...
<?php
	...
	function get_calendar() {
        ...
        $ajax=false; //set to true if you want to enable AJAX. Disable for integration with mappress  (poleouestEventMapDisplay)
        if ($ajax)
        {
...
function wb_get_future_posts( $query ) {
     ...
     if ((  is_date() || is_day() ||is_category()) &&  false == $query->query_vars['suppress_filters'] )
        {
...
$atts = array('width'=>300, 'height'=>250, "show"=>"current","marker_link"=>false, "marker_title"=>"postdate","marker_body"=>  "excerpt" , "initialopeninfo"=>false, "zoom"=>"7","center_lat"=>"48.2765002", "center_lng"=>"-3.5711409", 'autocenter'=>false); 
 
 $mapcode= $mappress->shortcode_mashup($atts);
wp_reset_query();
